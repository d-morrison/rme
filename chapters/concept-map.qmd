...
# together), then override each node's radius to be inversely proportional to
# its descendant count so the most foundational nodes land near the center.
fr_layout <- create_layout(core, layout = "fr", niter = 2000)
n_desc_vals <- V(core)$n_desc
max_desc <- max(n_desc_vals, 1L)
fr_angle <- atan2(fr_layout$y, fr_layout$x)
# sqrt transform spreads the radial distribution: n_desc = max_desc → r ≈ 0
# (center), n_desc = 0 → r = 1 (periphery).
target_r <- sqrt(1 - n_desc_vals / max_desc)
layout <- fr_layout
...
ggraph(layout) +
  ...
  geom_node_label(
    aes(label = title, fill = type, size = n_desc),
    color = "white",
    fontface = "bold",
