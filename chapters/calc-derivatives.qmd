:::{#thm-deriv-const}
#### Constant rule

$$\deriv{x}c = 0$$

:::

{{< slidebreak >}}

:::{#thm-deriv-const-factor}
#### Constant factor rule

If $a$ is constant with respect to $x$,
then:

$$\deriv{x}ay = a \derivf{y}{x}$$

:::

{{< slidebreak >}}

:::{#thm-deriv-polynomial}
#### Power rule

$$\deriv{x}x^q = qx^{q-1}$$

:::

{{< slidebreak >}}

:::{#thm-deriv-log}

#### Derivative of natural logarithm

$$\dlogf{x} = \frac{1}{x} = x^{-1}$$

:::

{{< slidebreak >}}

:::{#thm-deriv-exp}
#### Derivative of exponential

$$\dexpf{x} = \expf{x}$$

:::

{{< slidebreak >}}

:::{#thm-product-rule}
#### Product rule

$$(ab)' = ab' + ba'$$

:::

{{< slidebreak >}}

:::{#thm-quotient-rule}
#### Quotient rule

$$(a/b)' = a'/b - (a/b^2)b'$$

:::

{{< slidebreak >}}

:::{#thm-chain-rule}
#### Chain rule

$$\ba
\derivf{a}{c} &=
\derivf{a}{b} \derivf{b}{c}
\\
&= \derivf{b}{c} \derivf{a}{b}
\ea
$$

or in [Euler/Lagrange notation](https://en.wikipedia.org/wiki/Notation_for_differentiation#Lagrange's_notation):

$$(f(g(x)))' = g'(x) f'(g(x))$$

:::

{{< slidebreak >}}

:::{#cor-deriv-log-chain}

#### Chain rule for logarithms

$$
\deriv{x}\log{f(x)} = \frac{f'(x)}{f(x)}
$$

:::

::: proof
Let $u = f(x)$.
By the chain rule (@thm-chain-rule):

$$
\ba
\deriv{x}\log{f(x)}
&= \paren{\deriv{u}\log(u)} \cdot \paren{\deriv{x}f(x)} && \text{(by @thm-chain-rule)} \\
&= \frac{1}{u} \cdot f'(x) && \text{(by @thm-deriv-log)} \\
&= \frac{f'(x)}{f(x)} && \text{(substituting } u = f(x) \text{)}
\ea
$$
:::

---
