...
$\lincomp(\vx) = \dprodf{\vx}{\vb}$ through a chain of one-to-one transformations.
...
to the linear predictor $\lincomp(\vx) = \dprodf{\vx}{\vb} + \lincomp_0$,
where $\lincomp_0$ is the intercept term for each model
($\lincomp_0 = \mu(0)$ for linear/logistic,
$\lincomp_0 = \logf{\rate(0)}$ for Poisson,
$\lincomp_0(t) = \logf{\haz_0(t)}$ for Cox PH).
