...
format:
  ...
  docx:
    output-file: classification-handout.docx
...
In the following sections, we define the key performance measures of a diagnostic test
...
This target probability is the positive predictive value:
$$
\pmf{D \mid +} = \, ?
$$
...
This lower-than-expected probability arises because the disease prevalence is relatively low (7%), so false positives make up a meaningful fraction of all positive tests.
...
\ba
\pmf{D \mid +}
&= \frac{\pmf{+ \mid D} \cd \pmf{D}}{\pmf{+}}
&& \text{(Bayes' theorem)}
\\
&= \frac{\pmf{+ \mid D} \cd \pmf{D}}{\pmf{+ \mid D} \cd \pmf{D} + \pmf{+ \mid \neg D} \cd \pmf{\neg D}}
&& \text{(law of total probability for }\pmf{+}\text{)}
\\
&= \frac{\pmf{D}}{\pmf{D} + \frac{\pmf{+ \mid \neg D}}{\pmf{+ \mid D}} \cd \pmf{\neg D}}
&& \text{(divide numerator and denominator by }\pmf{+ \mid D}\text{)}
\\
&= \frac{1}{1 + \frac{\pmf{+ \mid \neg D}}{\pmf{+ \mid D}} \cd \frac{\pmf{\neg D}}{\pmf{D}}}
&& \text{(divide numerator and denominator by }\pmf{D}\text{)}
\\
&= \frac{1}{1 + \frac{1 - \text{spec}}{\text{sens}} \cd \frac{1 - \text{prev}}{\text{prev}}}
&& \text{(substitute definitions of sensitivity, specificity, and prevalence)}
\ea
$$
