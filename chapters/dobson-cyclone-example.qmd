...
lik <- function(lambda, y = dobson::cyclones$number, n = length(y)) {
...
loglik <- function(lambda, y = dobson::cyclones$number, n = length(y)) {
...
{{< include _subfiles/dobson-cyclone-example/_exr-pois-cyclone-score.qmd >}}
...
score <- function(lambda, y = dobson::cyclones$number, n = length(y)) {
  (sum(y) / lambda) - n
}
...
hessian <- function(lambda, y = dobson::cyclones$number, n = length(y)) {
  -sum(y) / (lambda^2)
}
...
{{< include _subfiles/dobson-cyclone-example/_sec_iterative_maximization.qmd >}}
...
This approximation is equivalent to estimating the log-likelihood with a second-order Taylor polynomial:
