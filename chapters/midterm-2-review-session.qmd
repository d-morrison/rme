...
```{r}
#| label: km-na-calc
KM_est <- survfit(surv ~ 1, type = "kaplan-meier", data = data)
...
**most recent event time at or before 17**, which is $t = 15$ (the value did
not change at the $15^+$ censoring). So:

$$\hat S_{KM}(17) = \hat S_{KM}(15) = 0.714,
\qquad
\hat S_{NA}(17) = \hat S_{NA}(15) = 0.734$$
...
| # | Topic | The fix |
|:-:|:------|:--------|
| 1 | Risk set after censoring (@sec-risk-set) | Remove **everyone** who has an event *or* is censored from later risk sets. |
| 2 | Nelson-Aalen (@sec-na-exp) | Sum hazards $d_j/n_j$, then $\hat S_{NA} = \exp\{-\hat\Lambda\}$. |
| 3 | Reading $\hat S(t)$ (@sec-step-function) | Step function: use the most recent event time $\le t$; give the number. |
| 4 | Median survival (@sec-median) | First $t$ with $\hat S(t) \le 0.5$ — not a mean, not an exponential fit. |
| 5 | Role of censoring (@sec-censoring-role) | Denominator until censoring; never the event numerator. |
| 6 | Writing the model (@sec-write-model) | Name **and** apply every assumption; baseline hazard stays unspecified. |
| 7 | Interpreting an HR (@sec-interpret) | Magnitude + reference + adjustment + significance; HR $\ne$ risk/odds. |
| 8 | HR for a non-unit change (@sec-hr-multiunit) | $HR^{\,c} = \exp\{c\beta\}$ — exponentiate, don't multiply. |
| 9 | Comparing two coefficients (@sec-wald) | $\V(\hat\beta_H - \hat\beta_L) = \V_H + \V_L - 2\,\Covt$. |
| 10 | CI for an HR (@sec-ci-hr) | Build on the log scale, then exponentiate the endpoints. |
