...
::: {.callout-tip}
The mistakes in this chapter are organized so that the most consequential and most
...
2. Computing the cumulative hazard correctly but then forgetting the final
   step, $\hsnaaf{t} = \exp{-\hat\Lambda(t)}$, and reporting the
...
$$\hsnaaf{t} = \exp{-\hat\Lambda(t)}$$

For example, at $t = 10$:

$$\hat\Lambda(10) = \frac{1}{7} = 0.143,
\qquad
\hsnaaf{10} = \exp{-0.143} = 0.867$$

Contrast this with Kaplan-Meier, which multiplies conditional survival
probabilities:

$$\hskmf{t} = \prod_{j: t_j \le t}\left(1 - \frac{d_j}{n_j}\right),
\qquad
\hskmf{10} = 1 - \tfrac{1}{7} = 0.857$$

The two estimators are close but **not** identical, and
$\hsnaaf{t} \ge \hskmf{t}$ always. Reporting the same numbers for both,
or reporting $\hat\Lambda$ where $\hsnaa$ was asked for, loses credit.
:::
...
$$\hskmf{17} = \hskmf{14} = 0.714,
\qquad
\hsnaaf{17} = \hsnaaf{14} = 0.734$$
...
- $\hskmf{t}$ first reaches $\le 0.5$ at $t = 18$ (where it drops to
  $0.476$), so the KM median is **18 months**.
- $\hsnaaf{t}$ first reaches $\le 0.5$ at $t = 21$ (where it drops to
...
No distributional assumption (exponential or otherwise) is used: the Kaplan–Meier estimator is a
...
later risk sets. This mechanism is exactly what lets Kaplan-Meier use partial information
...
estimates (on the $\log(HR)$ scale). The relevant entries are:
...
> **nonsmokers** ($\widehat{HR} = 1.43$); this hazard ratio is statistically significant at
...
> covariates; this hazard ratio is highly significant ($p < 0.001$, CI $(1.63, 2.91)$).
...
This mistake was the **single most consequential Part 2 error**. To test
...
- Reusing the wrong SE from the Wald test in @sec-wald.
:::
...
| 2 | 1.1 | Nelson-Aalen (@sec-na-exp) | Sum hazards $d_j/n_j$, then $\hsnaa = \exp{-\hat\Lambda}$. |
