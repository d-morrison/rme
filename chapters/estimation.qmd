...
format:
  ...
  docx:
    output-file: estimation-handout.docx
...
heights, except for the two most extreme; specifically, if we re-order the
...
to the distributions of the *estimation errors* made by the resulting estimates.
...
The **estimation error** of an estimate $\hth$ of a true value $\th$
is the difference between
the estimate and the estimand $\th$:
...
See [Linear-model residual definitions and terminology](Linear-models-overview.qmd#sec-lm-residuals)
for residual definitions
...
$\mselr{\hth}$, is the expectation of the square
of the estimation error[^1]:

$$\mselr{\hth} \eqdef \E{\sqf{\erf{\hth}}}$$
:::
...
$$\bias{\hth} \eqdef \E{\erf{\hth}}$$ {#eq-bias-def}
:::
...
\ba
\bias{\hth} 
&= \E{\erf{\hth}} && \text{(by Definition @def-bias)} \\
&= \E{\hth - \theta} && \text{(by Definition @def-estimation-error)} \\
&= \E{\hth} - \E{\theta} && \text{(by linearity of expectation)} \\
&= \E{\hth} - \theta && \text{(since $\theta$ is constant)}
\ea
$$
...
The **standard error** of an estimator $\hth$ is just the [standard deviation](probability.qmd#def-sd) of $\hth$;
specifically:
...
"Standard error" is a confusing concept in a few ways.
First of all,
it isn't even defined as a characteristic of the [estimation error](#def-estimation-error),
$\erf{\hth}$!
Moreover,
it is just a synonym for standard deviation,
so it seems like a redundant concept.
However,
standard errors help us construct p-values and confidence intervals,
so they come up a lot - often enough to give them their own name.

We can relate standard error to estimation error,
by rearranging the result from @thm-mse-bias-variance:
...
\ea
$$
So the variance of the estimator is equal to the variance of the estimation error,
and the standard error is equal to the standard deviation
...
(this equality is equivalent to @eq-unbiased-mse)
:::
