...
$$\theta(\vx_j) = \expf{\dprod{\vx_j}{\hat{\vb}}}$$
...
For each subject $i$ who experienced an event (not censored) and for
each predictor $k$,
the **Schoenfeld residual** is defined as [@grambsch1994proportional]:

$$
r^S_{ik} =
x_{ik}-\frac{\sum_{j\in \mathcal{R}(t_i)} x_{jk}\,\theta(\vx_j)}{\sum_{j\in \mathcal{R}(t_i)} \theta(\vx_j)}
$$

where $\mathcal{R}(t_i)$ is the risk set (@def-risk-set) at event time $t_i$ of subject $i$,
$x_{jk}$ is the value of predictor $k$ for subject $j$,
and $\theta(\vx_j)$ is the risk score (@def-risk-score) for subject $j$.

:::

Intuitively, $r^S_{ik}$ measures how unusual subject $i$'s covariate value
is relative to the risk-weighted average among everyone at risk when subject $i$ failed.
...
We can test for such a trend with the correlation between Schoenfeld residuals and a function of time.
...
relative to what the model predicted. They are bounded above by $1$
(a subject who died very early has residual close to $1 - 0 = 1$) but
unbounded below (a long-lived censored subject in a high-risk group can
have a very large negative residual). This asymmetry makes it harder to
spot unexpectedly early deaths, motivating the deviance residual.

We will use martingale residuals to examine the functional
...
where $\signt\{\cdot\}$ is the sign function, $r_i^M$ is the martingale
residual (@def-martingale-residual), and $r_i^{CS}$ is the Cox-Snell
residual (@def-CS-resid).

:::

Deviance residuals are roughly centered on 0 with approximate standard
deviation 1. By symmetrizing the skewed martingale residuals, they make
it equally easy to identify unexpectedly early events (large positive
$r^D_i$) and unexpectedly long survival (large negative $r^D_i$).
...
$|\text{dfbeta}_{ik}| > 2/\sqrt{n}$, analogous to Cook's distance in
linear regression.

```{r}
#| label: fig-dfbeta-hodg
#| fig-cap: "dfbeta values for each coefficient in `hodg_cox2`"
#| code-fold: true
hodg_dfb |>
  as.data.frame() |>
...
The most influential observations across all coefficients:

-   **Observation 1**: extreme dfbeta for graft type, disease type, score,
    and their interaction. This subject had an early death with a high
    Karnofsky score in a low-risk group.
-   **Observations 15 and 16**: large dfbeta for waiting time (dichotomized),
    likely reflecting sensitivity to the 80-day cut point.
-   **Observations 18 and 29**: top dfbeta values for Karnofsky score; also
    flagged by deviance residuals as unexpectedly early deaths.
...
The diagnostics above reveal two key concerns in `hodg_cox2`:

1. **Non-proportionality** of the `gtype:dtype` interaction and `score`,
   flagged by `cox.zph` in the original model.
2. **Influential observations**: subjects 1, 15, 16, 18, and 29 have
   large dfbeta values for specific coefficients.
...
(and its interaction with graft type) is not constant over time. One
remedy is a **stratified Cox model** that allows separate baseline
hazards for each disease type, absorbing between-disease trajectory
differences into two free baseline functions, while still estimating
proportional effects of graft type, Karnofsky score, and waiting time.
...
Stratifying by disease type removes the between-disease baseline
difference from the proportional-hazards stratum, so the remaining
terms should satisfy PH more closely.
...
The stratified model resolves the PH violation by allowing disease-specific
baseline hazards. The trade-off: we can no longer directly estimate a
disease-type hazard ratio, as that effect is absorbed into the baseline
functions. The graft-type coefficient remains interpretable as the
