...
For a Cox proportional hazards model with estimated coefficient vector $\hat{\vb}$,
the **risk score** (also called the **hazard multiplier** or **partial hazard**) for subject $j$
with covariate vector $\vx_j$ is:

$$\hrf{\vx_j} = \expf{\dprod{\vx_j}{\hat{\vb}}}$$

:::

:::{#thm-hr-constant}

#### Hazard Ratio Is Time-Constant Under Proportional Hazards

Under the proportional hazards assumption,
the hazard ratio comparing subjects with covariate vectors $\vx$ and $\vxs$ is:

$$\hr(t \mid \vx : \vxs) = \frac{\hrf{\vx}}{\hrf{\vxs}}$$
...
The Cox proportional hazards model specifies the hazard at time $t$ for a subject with covariates $\vx$ as:

$$\haz(t \mid \vx) = \haz_0(t) \cdot \hrf{\vx}$$

where $\haz_0(t)$ is the baseline hazard.
Taking the ratio for two covariate vectors $\vx$ and $\vxs$:

$$
\hr(t \mid \vx : \vxs)
= \frac{\haz(t \mid \vx)}{\haz(t \mid \vxs)}
= \frac{\haz_0(t) \cdot \hrf{\vx}}{\haz_0(t) \cdot \hrf{\vxs}}
= \frac{\hrf{\vx}}{\hrf{\vxs}}
$$

The $\haz_0(t)$ terms cancel, leaving a ratio that is constant over $t$.
...
$$
\ba
r_i^D &= \signt\{r_i^M\}\sqrt{-2\left[ r_i^M+\delta_i\ln(\delta_i-r_i^M)  \right]}\\
      &= \signt\{r_i^M\}\sqrt{-2\left[ r_i^M+\delta_i\ln(r_i^{CS})  \right]}
\ea
$$

where $\signt\{\cdot\}$ is the sign function,
...
-   **dfbeta** for observation $i$ is the approximate change in the
    coefficient vector $\hat{\vb}$ if subject $i$ were removed from the data:
    $$\text{dfbeta}_i \approx \hat{\vb} - \hat{\vb}_{(-i)}$$
...
::: notes
A useful rule of thumb: examine observations where
$|\text{dfbeta}_{ik}| > 2/\sqrt{n}$,
analogous to Cook's distance in linear regression.
:::
...
### Checking PH in the current model
...
data.frame(
  ...
  LogLik = c(as.numeric(logLik(hodg_cox2)), as.numeric(logLik(hodg_cox3))),
  AIC = c(AIC(hodg_cox2), AIC(hodg_cox3))
) |>
