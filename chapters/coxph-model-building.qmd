...
$$
r^S_{ik} =
x_{ik}-\frac{\sum_{j\in \risksetf{t_i}} x_{jk}\,\hrf{\vx_j}}{\sum_{j\in \risksetf{t_i}} \hrf{\vx_j}}
$$
...
and $\hrf{\vx_j}$ is the risk score (@def-risk-score) for subject $j$.
...
$$
\ba
r_i^D &= \signf{r_i^M}\sqrt{-2\left[ r_i^M+\delta_i\ln(\delta_i-r_i^M)  \right]}\\
      &= \signf{r_i^M}\sqrt{-2\left[ r_i^M+\delta_i\ln(r_i^{CS})  \right]}
\ea
$$
