...
$$
r^S_{ik} =
x_{ik}-\frac{\sum_{j\in \risksetf{t_i}} x_{jk}\,\hat{\hr}\paren{\vx_j}}{\sum_{j\in \risksetf{t_i}} \hat{\hr}\paren{\vx_j}}
$$
...
and $\hat{\hr}\paren{\vx_j}$ is the estimated risk score (@def-estimated-risk-score) for subject $j$.
...
This may have to do with the discretization of waiting time.
...
This is the same data as the `drug6mp` data from `KMsurv`, 
but with two
