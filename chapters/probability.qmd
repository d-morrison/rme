...
When $\alpha>1$ the hazard is
increasing and when $\alpha < 1$ the hazard is decreasing.
...
is
$$Q(p) \eqdef \inf\{t:F(t)\ge p\}, \quad 0<p\le 1.$$
:::
...
\ba
\E{X}
&= \sum_{x\in \rangef{X}} x \cd \P(X=x) && \text{(definition of expectation for discrete r.v.)}
\\&= \sum_{x\in \set{0,1}} x \cd \P(X=x) && \text{(range of Bernoulli r.v. is } \{0, 1\} \text{)}
\\&= \paren{0 \cd \P(X=0)} + \paren{1 \cd \P(X=1)} && \text{(expand sum over } x = 0 \text{ and } x = 1 \text{)}
\\&= \paren{0 \cd (1-\pi)} + \paren{1 \cd \pi} && \text{(substitute Bernoulli PMF values)}
\\&= 0 + \pi && \text{(multiplication by 0 and 1)}
\\&= \pi && \text{(addition of 0)}
\ea
$$
...
Since $\pdf(u) \ge 0$, condition (a) of
...
See [@statproofbook:mean-nnrvar] for an alternative presentation of this proof.
...
but a fully rigorous proof needs the general change-of-variables theorem
for integrals against a pushforward measure —
approximating $g$ by simple functions and passing to the limit —
which is beyond Epi 204's scope
[@billingsley1995probability; @gut2013; @CaseBerg01; @wp:lotus].
...
The relevant condition is (a) when $h \ge 0$ and
...
The following calculation is what someone would *erroneously* write if
...
This calculation recovers $\E{XY}$ for *independent* uniforms ($\tfrac{1}{4}$),
...
\ba
\E{\E{Y \mid X}}
&= \sum_{x \in \rangef{X}} \E{Y \mid X=x} \cd \P(X=x) && \text{(definition of expectation of } \E{Y \mid X} \text{)}
\\&= \sum_{x \in \rangef{X}} \paren{\sum_{y \in \rangef{Y}} y \cd \P(Y=y \mid X=x)} \cd \P(X=x) && \text{(definition of conditional expectation } \E{Y \mid X=x} \text{)}
\\&= \sum_{y \in \rangef{Y}} y \cd \sum_{x \in \rangef{X}} \P(Y=y \mid X=x) \cd \P(X=x) && \text{(exchange order of summation)}
\\&= \sum_{y \in \rangef{Y}} y \cd \P(Y=y) && \text{(law of total probability over } X \text{)}
\\&= \E{Y} && \text{(definition of expectation of } Y \text{)}
\ea
$$
...
case, **Fubini's theorem**); this theorem requires $\E{\abs{Y}} < \infty$,
...
This identity is the tower rule
...
\begin{aligned}
\Var{X}
&\eqdef \E{[\devn(X)]^2} && \text{(definition of variance)} \\
