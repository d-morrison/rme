...
measure-theoretic form (@thm-fubini-tonelli),
...
so @thm-fubini-tonelli yields the corollaries below directly.

{{< slidebreak >}}

:::{#cor-fubini-prob}

### Probability-measure form (independent variables; corollary of @thm-fubini-tonelli)
...
Probability measures are finite (hence σ-finite),
so the σ-finiteness hypothesis of @thm-fubini-tonelli is automatic;
under either (a) or (b) above the theorem applies and yields both
iterated forms.
...
$f_{X,Y}$ on $[0, 1]^2$ exists**, and in particular the joint
distribution is **not** the product measure $\P_X \otimes \P_Y$
that @cor-fubini-prob assumes.
...
The lesson is that the independence hypothesis of @cor-fubini-prob is
*structural*, not just notational: without it, there is no joint
density to integrate against in the first place.
The correct approach for *dependent* $(X, Y)$ that *do* admit a joint
density uses @cor-fubini-joint.
...
### Joint-density form (without independence; corollary of @thm-fubini-tonelli)
...
::: proof
Take both $\mu_1$ and $\mu_2$ to be Lebesgue measure on
$\rangef{X}$ and $\rangef{Y}$ respectively, and apply
@thm-fubini-tonelli to the integrand $h(x,y)\,f_{X,Y}(x,y)$ on
$\rangef{X} \times \rangef{Y}$.
The relevant hypothesis is (a) when $h \ge 0$
and (b) when $\E{\abs{h(X, Y)}} < \infty$.
When the joint density's support is not all of
$\rangef{X} \times \rangef{Y}$, the convention that $f_{X,Y}$ is
extended by zero outside its support means the integrand
$h(x,y)\,f_{X,Y}(x,y)$ vanishes there, so the integral over the full
product domain agrees with the integral over the support;
@thm-fubini-tonelli then expresses it as either iterated form above.
...
### A Fubini–Tonelli proof of @thm-surv-mean (continuous case) {#sec-fubini-proof-surv-mean}

@thm-surv-mean states that for any nonnegative random variable $T$,
$\E{T} = \int_0^\infty \surv(t)\,dt$.
When $T$ has a density $\pdf$, this follows from applying Tonelli's
theorem (hypothesis (a) of @thm-fubini-tonelli) to the function
...
Since $\pdf(u) \ge 0$, hypothesis (a) of @thm-fubini-tonelli
...
hypothesis (b) of @thm-fubini-tonelli (the absolute-integrability
