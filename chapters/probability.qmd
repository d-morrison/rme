...
The Riemann version of Fubini's theorem (@thm-fubini, stated in the
math-prereqs chapter) lets us switch the order of integration for
continuous integrands on simple regions.
For expectations against probability measures we need its
measure-theoretic generalization (mentioned in the notes following
@thm-fubini), which holds on any σ-finite measure space.
The σ-finiteness hypothesis is automatic for probability measures
(every probability measure is finite, hence σ-finite),
so the generalization yields the corollaries below directly.

{{< slidebreak >}}

:::{#cor-fubini-prob}

### Probability-measure form (independent variables; corollary of @thm-fubini)

Let $X$ and $Y$ be independent random variables
with probability measures $\P_X$ and $\P_Y$,
and let $h : \rangef{X} \times \rangef{Y} \to \mathbb{R}$
be measurable.
If either
...
$$
\E{h(X, Y)}
= \int_{\rangef{X}} \paren{\int_{\rangef{Y}} h(x, y)\,d\P_Y(y)}\,d\P_X(x)
= \int_{\rangef{Y}} \paren{\int_{\rangef{X}} h(x, y)\,d\P_X(x)}\,d\P_Y(y).
$$
...
By independence, the joint distribution of $(X, Y)$ is the product measure
$\P_X \otimes \P_Y$, so
$\E{h(X, Y)} = \int_{\rangef{X} \times \rangef{Y}} h(x, y)\,d(\P_X \otimes \P_Y)$.
Since probability measures are finite (hence σ-finite),
the measure-theoretic generalization of @thm-fubini
(see the notes below @thm-fubini) applies and gives both iterated
forms.
...
We apply @cor-fubini-prob with $h(x, y) = xy$.
Since $X$ and $Y$ have densities $f_X(x) = 1$ and $f_Y(y) = \tfrac{1}{2}$,
we have $d\P_X(x) = f_X(x)\,dx = dx$ and $d\P_Y(y) = f_Y(y)\,dy = \tfrac{1}{2}\,dy$:
...
The independence condition in @cor-fubini-prob is essential.
Without it, applying the product-measure formula gives the wrong answer.
...
**Incorrectly applying the product-measure formula:**
...
@thm-fubini.
Rather, the error is using the *wrong measure*: treating the joint
distribution as $\P_X \otimes \P_Y$ when in fact $X$ and $Y$ are
perfectly correlated, so the actual joint distribution lies on the
diagonal $\{(x, x) : x \in [0, 1]\}$ and is not a product measure.

Pretending $X$ and $Y$ are independent, with marginals
both $\mathrm{Uniform}(0, 1)$ (densities $f_X = f_Y = 1$):
...
Since $\tfrac{1}{4} \neq \tfrac{1}{3}$,
applying the independence formula when $X$ and $Y$ are dependent
gives the wrong answer.
The correct approach for dependent $(X, Y)$ uses the joint density
(see @cor-fubini-joint).
...
### Joint-density form (without independence)

Let $(X, Y)$ be jointly distributed continuous random variables
with joint density $f_{X,Y}$
supported on a (possibly non-rectangular) subset of
$\rangef{X} \times \rangef{Y}$
and extended by zero to the full product,
...
then the expectation of $h(X, Y)$ can be written as a double integral
against the joint density, with the order of integration exchangeable:

$$
\E{h(X, Y)}
= \int_{\rangef{X}}\paren{\int_{\rangef{Y}} h(x, y)\,f_{X,Y}(x, y)\,dy}\,dx
= \int_{\rangef{Y}}\paren{\int_{\rangef{X}} h(x, y)\,f_{X,Y}(x, y)\,dx}\,dy.
$$
...
::: proof
Apply the measure-theoretic generalization of @thm-fubini
(see the notes below @thm-fubini)
to the integrand $h(x,y)\,f_{X,Y}(x,y)$, integrated against
Lebesgue measure.
If the joint density's support is not a rectangle, extend $f_{X,Y}$ by
zero outside its support to an enclosing simple region;
@thm-fubini Part 1 then expresses the integral as the iterated form
above.
Independence is not required.
When $X$ and $Y$ are independent, $f_{X,Y}(x,y) = f_X(x)\,f_Y(y)$,
and this corollary reduces to @cor-fubini-prob.
:::
...
#### Expectation using a joint density (dependent variables)
...
### Survival-function mean (corollary of @thm-fubini)

Let $T \ge 0$ be a continuous nonneg random variable with
density $\pdf$ and survival function $\surv(t) = \P(T > t)$.
Then:
...
Since $\pdf(u) \ge 0$, Tonelli's theorem
(the nonnegativity case of the measure-theoretic generalization of
@thm-fubini, see the notes below @thm-fubini)
allows exchanging the order of integration over the unbounded triangular
region $D = \{(t, u) : 0 \le t \le u < \infty\}$:
...
Let $T \sim \mathrm{Exponential}(\lambda)$, so $\surv(t) = e^{-\lambda t}$
for $t \ge 0$.
By @cor-fubini-survfn:
...
&= \sb{-\frac{1}{\lambda}e^{-\lambda t}}_0^\infty\\
&= \frac{1}{\lambda},
\ea
$$
...
(the absolute-integrability case of the measure-theoretic generalization
of @thm-fubini; this requires $\E{\abs{Y}} < \infty$,
