...
### Corollary for probability measures
...
their joint measure is the product measure
$\P_X \otimes \P_Y$,
and for the random variable $h(X, Y)$ induced by $h$,
the expectation under the joint distribution satisfies
$\E{h(X, Y)} = \int_{\rangef{X} \times \rangef{Y}} h(x, y)\,d(\P_X \otimes \P_Y)$.
Since $\P_X$ and $\P_Y$ are finite (hence σ-finite) measures on Borel σ-algebras,
applying the general form of @thm-fubini gives the equivalent iterated forms:
...
integrating against the marginal densities
$f_X(x) = 1$ for $x \in [0, 1]$ and $f_Y(y) = \tfrac{1}{2}$ for $y \in [0, 2]$:
...
both $\mathrm{Uniform}(0, 1)$:

$$
\ba
\int_0^1\!\int_0^1 xy \cd 1 \cd 1\,dy\,dx
...
### Corollary for joint probability measures (without independence)
...
This follows from applying the Lebesgue special case of @thm-fubini to the integrand $h(x,y)\,f_{X,Y}(x,y)$.
Independence is not required.
When $X$ and $Y$ are independent, $f_{X,Y}(x,y) = f_X(x)\,f_Y(y)$,
recovering @cor-fubini-prob.
