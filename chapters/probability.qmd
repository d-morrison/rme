...
This example's support is finite, so reordering the double sum above
is justified by ordinary commutativity and associativity of addition:
there are only four terms, and adding four numbers in a different order
cannot change their total.
@cor-fubini-joint's guarantee isn't actually doing any work here.
@exm-fubini-joint-disc-infinite below gives a case
where the support is countably infinite instead,
so the guarantee is load-bearing.
...
::::{#exm-fubini-joint-disc-infinite}
...
likewise for $Y$).
Unlike @exm-fubini-joint-disc, the support here is countably infinite.
The joint PMF is
...
@exm-fubini-joint-mixed-infinite below gives $Y$ a countably infinite
...
::::{#exm-fubini-joint-mixed-infinite}
...
given $Y = y$, let $X \mid Y = y \sim \mathrm{Uniform}(0,\, y+1)$.
Unlike @exm-fubini-joint-mixed, $Y$'s range here is countably infinite.
Compute $\E{X}$ using @cor-fubini-joint with $\mu_X = \text{Lebesgue measure}$,
$\mu_Y = \text{counting measure}$, and $h(x, y) = x$.
...
sum-of-integrals below is valid --- unlike @exm-fubini-joint-mixed,
...
using the same geometric-series facts as @exm-fubini-joint-disc-infinite
(e.g. @CaseBerg01).
