...
$\mu_X = \mu_Y = $ counting measure and $h(x,y) = x + y$.
...
Compute $\E{X}$ using @cor-fubini-joint with $\mu_X = $ Lebesgue measure,
$\mu_Y = $ counting measure, and $h(x, y) = x$.
...
plotly::plot_ly(df, x = ~x, y = ~density, color = ~label,
  colors = c("steelblue", "tomato")) |>
  plotly::add_lines() |>
  plotly::layout(
    xaxis = list(title = "x"),
