...
* An alternative to using a generalized linear model with a log link is to transform the outcome using the log transformation. This outcome-transformation device works well with continuous measurement data and may be usable in other cases, but it cannot be used for 0/1 binary data or for count data that may be 0.
