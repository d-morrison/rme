...
In generalized linear modeling, we specify a linear predictor $\vx \cdot \vb$ of the same form as in linear regression.
...
that maps the expected outcome $\mu = \E{Y \mid \vX}$
...
* Standard linear regression uses the identity link function $g(\mu) = \mu$
and assumes a Gaussian response distribution.

* When predicting an inherently positive outcome,
a log link function $g(\mu) = \logf{\mu}$ is often appropriate,
