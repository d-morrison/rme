...
the standard Poisson assumption $\Var{Y \mid \vX=\vx} = \Expp[Y \mid \vX=\vx]$
is violated,
...
It is an *inference method* rather than a model structure:
rather than specifying a complete probability distribution
and fitting by maximum likelihood,
it specifies only the mean-variance relationship
$\Var{Y \mid \vX=\vx} = \theta \mu(\vx, t)$,
...
This method-of-moments approach provides valid standard errors
and $p$-values when overdispersion is multiplicative,
though these are model-based errors rescaled by $\sqrt{\eth}$
rather than sandwich estimates,
so they rely on that assumed mean-variance relationship holding.
