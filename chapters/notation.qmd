...
format:
  ...
  docx:
    output-file: notation-handout.docx
...
| $\indic{A}(x)$, $\indicp{P}$ | indicator function (@sec-indicator-functions): $1$ if condition holds, $0$ otherwise | `\indic{A}(x)`, `\indicp{P}` |
...
{{< include _subfiles/notation/_sec-indicator-functions.qmd >}}
