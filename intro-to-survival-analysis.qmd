...
$$\surv(t) \eqdef \Pr(T > t)$${#eq-def-surv}
...
\begin{aligned}
\surv(t) &= 1 - \cdf(t)\\
&= \text{exp}\left\{-\cuhaz(t)\right\}\\
\surv'(t) &= -f(t)\\
\cuhaz(t) &= -\text{log}\left\{\surv(t)\right\}\\
\cuhaz'(t) &= \haz(t)\\
...
&= \text{log}\left\{S(0)\right\} - \text{log}\left\{\surv(t)\right\}\\
&= \text{log}\left\{1\right\} - \text{log}\left\{\surv(t)\right\}\\
&= 0 - \text{log}\left\{\surv(t)\right\}\\
&=-\text{log}\left\{\surv(t)\right\}
\end{aligned}
$$
...
\\ \tf \text{Var}\left(\hat \surv(t)\right)
...
$$\cuhaz(t)\eqdef  \int_0^t \haz(t) dt$$ Since
$\haz(t) = -\deriv{t}\text{log}\left\{\surv(t)\right\}$ as shown above, we
have:

$$
\cuhaz(t)=-\text{log}\left\{S\right\}(t)
$$
