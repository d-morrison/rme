...
Since $\haz(t) = \deriv{t}\cb{-\log{S(t)}}$ (see @thm-h-logS), we also have:
...
$$
\haz(t) \xrightarrow[]{\int_{u=0}^t h(u)du} H(t) 
\xrightarrow[]{\exp{-H(t)}} S(t)
\xrightarrow[]{1-S(t)} F(t)
$$

$$
\haz(t) \xleftarrow[\deriv{t}H(t)]{} H(t) 
...
- The log–rank test uses a test statistic that compares KM curves across all survival times.
...
\ba
\hat{E}_{t,x} 
&= \frac{\sum_{t_i = t} d_i}{\sum_i\indiccb{t_i \ge t}} \sum_i\indiccb{t_i \ge t \and X_i=x}
\ea
$$
...
$$H(t)\eqdef  \int_0^t \haz(t) dt$$ Since
$\haz(t) = -\deriv{t}\text{log}\left\{S(t)\right\}$ as shown above, we
have:
