...
Suppose we want to try forcing the intercept to go through 0:
:::

```{r}
hers <- fs::path_package("rme", "extdata/hersdata.dta") |> haven::read_dta()
...
Fitted values plus residuals for interaction model for `birthweight` data
...
how do check for errors in the mean structure assumption?
:::
