...
:::
$$ E[Y|A=a, M=m] = \mu(a,m) = \beta_0 + \beta_M m + \beta_A a $$

Slope (of the mean with respect to age) for males:
$$
\begin{aligned}
\frac{d}{da}\mu(a,1) &= \frac{d}{da}(\beta_0 + \beta_M 1 + \beta_A a)\\
...
$$
\begin{aligned}
\frac{d}{da}\mu(a,0) &= \frac{d}{da}(\beta_0 + \beta_M 0 + \beta_A a)\\
...
$$
\begin{aligned}
\frac{d}{da}\mu(a,m) &= \frac{d}{da}(\beta_0 + \beta_M m + \red{\beta_A} a)\\
...
:::{#thm-beta-m-interpretation}
$$ \beta_M = E[Y|a,1] - E[Y|a,0] $$
...
$$
\begin{aligned}
E[Y|a,1] &= \beta_0 + \beta_M 1 + \beta_A a \\
& = \beta_0 + \beta_M + \beta_A a \\
E[Y|a,0] &= \beta_0 + \beta_M 0 + \beta_A a \\
& = \beta_0 + \beta_A a
\end{aligned}
$$
So: 
$$
\begin{aligned}
E[Y|a,1] - E[Y|a,0] &= (\beta_0 + \beta_M + \beta_A a) - (\beta_0 + \beta_A a) \\
&= \beta_M
\end{aligned}
$$
:::
...
\begin{aligned}
\E{Y|0,0} &= \mu(0,0)\\
&= \beta_0 + \beta_M 0 + \beta_A 0\\
&= \beta_0
\\ \beta_0 &= \E{Y|0,0} = \mu(0,0)
\end{aligned}
$$
...
In this case, the coefficient $S$ is interacting with $A$. 
...
What is the interpretation of $\beta_{M}$?
:::
...
$\beta_{M} = \mu(0,1) - \mu(0,0)$ is the difference in mean birthweight between males with gestational age 0 weeks and females with gestational age 0 weeks.
:::

::: {#exr-interaction-coef}
What is the interpretation of $\beta_{AM}$?
:::
...
$$
\begin{aligned}
\deriv{a}\mu(a,1) &= \deriv{a}\left(\beta_0 + \beta_M\cdot1 + \beta_A\cdot a + \red{\beta_{AM}}\cdot 1 \cdot a\right)\\
...
$$
\begin{aligned}
 E[Y|a+1,1] - E[Y|a,1] = &\beta_0 + \beta_M 1 + \beta_A(a+1) + \red{\beta_{AM}}1(a+1)\\ 
...
$$
\begin{aligned}
\deriv{a}\mu(a,0) &= \deriv{a}\left(\beta_0 + \beta_M\cdot0 + \beta_A\cdot a + \red{\beta_{AM}}\cdot 0 \cdot a\right)\\
...
$$
\begin{aligned}
 E[Y|a+1,0] - E[Y|a,0] = &\beta_0 + \beta_M0 + \beta_A(a+1) + \red{\beta_{AM}}0(a+1) \\
...
$$
\begin{aligned}
 \deriv{a}\mu(a,1) - \deriv{a}\mu(a,0) &= \beta_A + \red{\beta_{AM}} - \beta_A\\
 &= \red{\beta_{AM}}
\end{aligned}
$$
or
$$
\begin{aligned}
 (E[Y|a+1,1] - E[Y|a,1]) - (E[Y|a+1,0] - E[Y|a,0]) &= \beta_A + \red{\beta_{AM}} - \beta_A\\
 &= \red{\beta_{AM}}
\end{aligned}
$$
...
:::{#tbl-compare-coef-interps}
  $\mu(a,s)$    | $\beta_0 + \beta_M m + \beta_A a$ | $\beta_0 + \beta_M m + \beta_A a + \red{\beta_{AM} ma}$
----------------|------------------------------------|---------------------------------------------------
  $\beta_0$     | $E[Y\mid a,1] - E[Y\mid a,0]$      | $E[Y\mid A=0,1] - E[Y\mid A=0,0]$
  $\beta_A$     | $\deriv{a}\mu(a,m)$                | $\deriv{a}\mu(a,0)$
  $\beta_M$     | $E[Y\mid a,1] - E[Y\mid a,0]$      | $E[Y\mid A=0,1] - E[Y\mid A=0,0]$
  $\beta_{AM}$  |                                    | $\deriv{a}\mu(a,1) - \deriv{a}\mu(a,0)$
...
For example, in the `birthweight` data, there are $q = `r n_unique_bw`$ unique patterns (@tbl-bw-x-combos).
...
In a regression model $\p(y|x)$, 
the **predicted value** of $y$ given $x$ is the estimated mean of $Y$ given $X$:
...
For linear models, the predicted value can be straightforwardly calculated 
...
We can also get **prediction intervals** for 
...
$$Y_i|\vX_i \sim_{\red{\ind}} \blue{\Normal}(\orange{\vX_i'\vb}, \green{\ss})$$
...
also known as the **deviation of an observation from its mean**,
is the difference between an observed value $y$ and its distributional mean:
...
Conversely, each observation $y$ can be viewed as an estimate of $\Exp{Y}$ 
