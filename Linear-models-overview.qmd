...
:::{.notes .smaller}
The below table compares the coefficient interpretations for the parallel lines and interaction models.
...
&= -\frac{1}{2\sigma^2}X'X
\ea$$

$\ell_{\beta, \beta'} ''(\beta, \sigma^2;\mathbf X,\vy)$  is negative definite at $\beta = (X'X)^{-1}X'y$, so $\hat \beta_{ML} = (X'X)^{-1}X'y$ is the MLE for $\beta$.
