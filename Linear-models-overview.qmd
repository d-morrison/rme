...
:::{.solution}
$\beta_A$ is the slope of mean birthweight (with respect to gestational age), holding sex constant:

$$
\begin{aligned}
\frac{d}{da}\mu(a,m) &= \frac{d}{da}(\beta_0 + \beta_M m + \beta_A a)\\
&= \left(\frac{d}{da}\beta_0 + \frac{d}{da}\beta_M m + \frac{d}{da}\beta_A a\right)\\
&= (0 + 0 + \beta_A) \\ 
&= \beta_A
\end{aligned}
$$
...
$\beta_A$ is the difference in mean birthweight per one-unit difference in gestational age, adjusting for sex:
:::
...
$$
\begin{aligned}
\E{Y|0,0} &= \beta_0 + \beta_M 0 + \beta_A 0\\
&= \beta_0
\\ \beta_0 &= \E{Y|0,0}
\end{aligned}
$$
...
$$
\begin{aligned}
 E[Y|a+1,1] - E[Y|a,1] = &\beta_0 + \beta_M 1 + \beta_A(a+1) + \beta_{AM}1(a+1)\\ 
  &- (\beta_0 + \beta_M 1 + \beta_A(a) + \beta_{AM}1(a))\\
 = &\beta_A + \beta_{AM}
 \end{aligned}
$$
...
$$
\begin{aligned}
 E[Y|a+1,0] - E[Y|a,0] = &\beta_0 + \beta_M0 + \beta_A(a+1) + \beta_{AM}0(a+1) \\
 &- (\beta_0 + \beta_M 0 + \beta_A(a) + \beta_{AM}0(a))\\
...
$$
\begin{aligned}
 (E[Y|a+1,1] - E[Y|a,1]) - (E[Y|a+1,0] - E[Y|a,0]) &= \beta_A + \beta_{AM} - \beta_A\\
 &= \beta_{AM}
\end{aligned}
$$
:::
...
----------------|------------------------------------|---------------------------------------------------
  ...
  $\beta_A$     | $E[Y\mid a+1,m] - E[Y\mid a,m]$    | $E[Y\mid a+1,0] - E[Y\mid a,0]$
  $\beta_M$     | $E[Y\mid a,1] - E[Y\mid a,0]$      | $E[Y\mid A=0,1] - E[Y\mid A=0,0]$
  $\beta_{AM}$  |                                    | $\begin{aligned}(&E[Y\mid a+1,1] - E[Y\mid a,1])\\ &- (E[Y\mid a+1,0] - E[Y\mid a,0])\end{aligned}$
