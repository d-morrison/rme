...
#| tbl-cap: "Regression parameter estimates for [Model @eq-lm-parallel] of `birthweight` data"
...
and you will see what other variables are interacting with the variable you are interested in.
In this case, the coefficient $S$ is interacting with $A$. 
So the slope of $Y$ with respect to $A$ depends on the value of $M$.
According to this model, there is no such thing as "*the* slope of birthweight with respect to age". 
...
Then: that coefficient is the difference in means per unit change in its corresponding coefficient, 
when the other collected variables are set to 0.
:::
...
log_LR <- (logLik(bw_lm2) - logLik(bw_lm1)) |> as.numeric() # nolint: object_name_linter
...
d_log_LR <- function(x, df = delta_df) dchisq(x, df = df)  # nolint: object_name_linter

chisq_plot <-
  ggplot() +
  geom_function(fun = d_log_LR) +
  stat_function(fun = d_log_LR, 
                xlim = c(log_LR, x_max), 
                geom = "area", 
                fill = "gray") +
  geom_segment(
...
```{r}
bw_X_unique <-  # nolint: object_name_linter
  bw |>
  count(sex, age)
...
2. Correct Functional Form: The conditional means have the structure $$E[Y|\vec X = \vec x] = \vec x'\beta$$
...
$$\am_{\theta} \llik(\th) + \lambda \sum_{j=1}^p|\beta_j|$$
