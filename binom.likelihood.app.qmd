...
Based on these calculations, we can conclude that the second derivative of the log-likelihood is negative. Thus the estimator $\hat{\pi}_{MLE} = \bar{x}$ is, in fact, the Maximum Likelihood Estimator of $\pi$.
...
# which the power plot indicates will be sufficient to search among
...
Compare the empirical results with the expected theoretical results.

::: {.callout-tip}

Instead of copy-pasting code, write a function that takes the data-generating $\pi$, the null hypothesis $\pi_0$, the sample size, and the number of simulations to run as inputs, and returns the preceding performance characteristics.
