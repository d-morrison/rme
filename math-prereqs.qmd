...
$$\derivf{\vec{z}}{\vec{x}} = \derivf{\vec{y}}{\vec{x}} \derivf{\vec{z}}{\vec{y}}$$

or in Euler/Lagrange notation:

$$(f(\vec{g}(\vx)))' = \vec{g}'(\vx) f'(\vec{g}(\vx))$$
...
$$\deriv{\vb}{\paren{\vec\eps\paren{\vb}\cdot \vec\eps\paren{\vb}}} = 2 \paren{\deriv{\vb}\vec{\eps}\paren{\vb}} \vec\eps\paren{\vb}$$

::: notes

See <https://en.wikipedia.org/wiki/Gradient#Relationship_with_Fr%C3%A9chet_derivative>

This is like the univariate chain rule, but the order matters now.
The version presented here is for the gradient (column vector derivative);
the total derivative (row vector derivative would be the transpose).
See <https://en.wikipedia.org/wiki/Gradient#Relationship_with_total_derivative>
