...
To find the MLE,
we set the score equal to zero.
...
Substitute $\eps_i = y_i - (\vx_i \cdot \vb)$:
...
Assume the design matrix $\underbrace{\mX}_{n \times p}$ has full column rank $p$.
This full column rank condition implies that the $p \times p$ matrix $\sum_{i=1}^n \vx_i \tp{\vx_i} = \underbrace{\tp{\mX}}_{p \times n} \underbrace{\mX}_{n \times p}$ is invertible.
To solve for $\vb$,
use $\vx_i (\vx_i \cdot \vb) = (\vx_i \tp{\vx_i})\vb$:

$$
\ba
\paren{\sumin \vx_i \tp{\vx_i}}\vb &= \sumin \vx_i y_i
\\
\vb &= \paren{\sumin \vx_i \tp{\vx_i}}^{-1}\sumin \vx_i y_i
\\
\hvb &= (\tp{\mX}\mX)^{-1}\tp{\mX}\vy
\ea
$$
