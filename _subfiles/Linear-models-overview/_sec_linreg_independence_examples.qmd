...
indep_exm <- tibble::tibble(
  time_index = time_index,
  x = x,
  y = y
)

indep_exm_lm <- lm(y ~ x, data = indep_exm)

summary(indep_exm_lm)
```
...
indep_exm |>
  dplyr::mutate(resid = resid(indep_exm_lm)) |>
...
#| code-fold: false
lmtest::dwtest(indep_exm_lm, alternative = "two.sided")
lmtest::bgtest(indep_exm_lm, order = 4)
```

In this example,
the residual-order plot and correlogram suggest serial dependence,
and both formal tests typically reject independence.
