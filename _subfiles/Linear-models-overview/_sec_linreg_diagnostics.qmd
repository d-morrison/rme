...
- The **y-axis** (Standardized Residuals) shows the observed standardized residual $r_i$.
  ...
  these plotted $r_i$ values are the ordered sample quantiles
  (order statistics)
  of the empirical standardized residual distribution
  at the corresponding plotting positions.
  So both labels are correct:
  `qqnorm()` calls them "Sample Quantiles"
  because ordered standardized residuals are sample quantiles,
  while `autoplot.lm()` calls those same values "Standardized residuals".
