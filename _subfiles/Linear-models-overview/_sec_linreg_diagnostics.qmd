...
By @def-resid-noise, $\err(Y) = Y - \Expf{Y}$.
Since $\err(Y)$ is an affine function of $Y$ (a linear function plus a constant),
and Gaussian distributions are closed under affine transformations,
if $Y \sim \Normal(\mu, \ss)$, then:

$$
\err(Y) = Y - \mu \sim \Normal(\mu - \mu, \ss) = \Normal(0, \ss)
$$

Conversely, since $Y = \Expf{Y} + \err(Y) = \mu + \err(Y)$,
if $\err(Y) \sim \Normal(0, \ss)$, then $Y = \mu + \err(Y) \sim \Normal(\mu, \ss)$.
...
$h_{ii} \to 0$ as $n \to \infty$, so $\Var{e_i} \approx \ss$ for large $n$.
...
so $\Var{e_i} = \ss(1 - h_{ii}) \to \ss$.
This limit holds under standard regularity conditions:
fixed number of predictors $p$,
and $\frac{1}{n}\tp{\mX}\mX \to \Sigma$ for some positive definite matrix $\Sigma$
(i.e., the design is asymptotically balanced),
which implies $h_{ii} = O(p/n) \to 0$.
...
In a Gaussian linear regression model,
the residuals $e_i$ are marginally normally distributed:

$$e_i \sim \Normal\paren{0,\; \ss(1 - h_{ii})}$$ {#eq-resid-dist-exact}
...
$$e_i \approx \Normal(0, \hat\ss)$$ {#eq-resid-dist-approx}
...
$$e_i = y_i - \hat y_i = \sum_{j=1}^n (\delta_{ij} - h_{ij})\, y_j$$
...
Therefore $e_i \sim \Normal(0, \ss(1 - h_{ii}))$.
