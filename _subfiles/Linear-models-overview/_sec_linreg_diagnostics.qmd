...
if $\hExpf{Y_i}$ is an [unbiased](estimation.qmd#sec-unbiased-estimators)
estimator of the mean $\Expf{Y_i}$ for each $i$, so that:

$$ \Expf{\hExpf{Y_i}} = \Expf{Y_i}$$
...
the deviations from the model-implied means
are conditionally i.i.d. Gaussian:

$$
\devn(Y_i) \mid \mX \siid N(0, \sigma^2)
$$
