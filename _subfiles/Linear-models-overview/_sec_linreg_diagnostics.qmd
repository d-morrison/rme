...
- The **y-axis** (Standardized Residuals) shows the observed standardized residual $r_i$.
  ...
  Let $r_{(1)} \le \cdots \le r_{(n)}$ be the ordered standardized residuals
  (the order statistics).
  The empirical sample-quantile function is
  $$\hat Q(p) = \inf\{t : \hat F_r(t) \ge p\}, \quad 0 < p < 1,$$
  where $\hat F_r$ is the empirical CDF of the standardized residuals.
  For plotting positions $p_i = (i - 0.5) / n$,
  the QQ plot uses points
  $$\left(\Phi^{-1}(p_i),\, r_{(i)}\right), \quad i = 1, \ldots, n.$$
  Since $r_{(i)} = \hat Q(p_i)$,
  each plotted standardized residual is exactly the sample quantile at $p_i$.
