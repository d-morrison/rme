...
- The **y-axis** (Standardized Residuals) shows the observed standardized residual $r_i$.
  ...
  these values are sample quantiles.
  Let $r_{(1)} \le \cdots \le r_{(n)}$ be the order statistics
  of the standardized residuals.
  Let $\hat F_r$ be their empirical CDF
  and $\hat Q_r$ the corresponding sample quantile function.
  For plotting positions $p_i = (i - 0.5) / n$,
  the QQ plot uses points
  $$\left(\Phi^{-1}(p_i),\, \hat Q_r(p_i)\right), \quad i = 1, \ldots, n,$$
  where $\hat Q_r(p_i)=r_{(i)}$ for $i=1,\ldots,n$.
  This holds because $p_i=(i-0.5)/n=(2i-1)/(2n)$,
  so $(i-1)/n<(2i-1)/(2n)\le i/n$.
  Therefore, each plotted standardized residual
  is exactly the sample quantile at $p_i$.
  So both labels are correct:
  `qqnorm()` emphasizes the quantile role ("Sample Quantiles"),
  while `autoplot.lm()` emphasizes the underlying values
  ("Standardized residuals").
