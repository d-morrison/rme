...
- **deviation**
  for deviation from a population,
  a subpopulation,
  or a model-implied mean
  (often called **error** in other sources,
  even though it doesn't involve an estimate;
...
with fitted values $\hat y = HY$,
if $\hExpf{Y}$ is an [unbiased](estimation.qmd#sec-unbiased-estimators)
estimator of the mean $\Expf{Y}$
and the errors are homoskedastic so that
$\Var{Y \mid X} = \ss I$,
then:
