...
$\Var{\vY \mid \mX} = \ss I_n$ where $I_n$ is the $n \times n$ identity matrix,
then,
...
\ba
\resid &= (I_n-H)Y
\\ \Var{\resid \mid \mX} &= \Var{(I_n-H)Y \mid \mX}
\\ &= (I_n-H)\Var{Y \mid \mX}(I_n-H)^\top
\\ &= (I_n-H)\ss I_n(I_n-H)^\top
\\ &= \ss(I_n-H)(I_n-H)^\top
\\ &= \ss(I_n-H)^2
\\ &= \ss(I_n - 2H + H^2)
\\ &= \ss(I_n-H) \qquad (\text{since } H^2 = H)
...
bw <-
  ...
  mutate(
    ...
    std_resid_lm2_alt =
    resid_lm2 / (sigma(bw_lm2) * sqrt(1 - hatvalues(bw_lm2)))
  )
