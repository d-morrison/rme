...
$$\Var{\resid_i} = \ss(1-h_{ii}) \approx \ss \text{ when } h_{ii} \text{ is small}$$ {#eq-var-resid-unbiased}
...
\\ &\approx \ss \text{ when } h_{ii} \text{ is small}
\ea
$$
:::
...
resid(bw_lm2) / (sigma(bw_lm2) * sqrt(1 - h_ii))
rstandard(bw_lm2)
```
...
rstandard_compare_plot <-
  tibble(
    x = resid(bw_lm2) / (sigma(bw_lm2) * sqrt(1 - hatvalues(bw_lm2))),
    y = rstandard(bw_lm2)
  ) |>
  ggplot(aes(x = x, y = y)) +
  geom_point() +
