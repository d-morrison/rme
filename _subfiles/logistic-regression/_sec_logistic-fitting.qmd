...
Fitted logistic regression model for `beetles` data
...
Fitted logistic regression model for `beetles` data, with `dose` centered
...
$$\sumin \cb{\ub{\vx_i}_{(p+1) \times 1} \ub{\paren{y_i - \expitf{\tp{\vx_i}\vb}}}_{1 \times 1} } = \mathbf{0}_{(p+1) \times 1}$$ {#eq-score-logistic}
...
Fitted model for `beetles` data

:::

:::{#tbl-beetles-glm-ungrouped-vcov}
...
Parameter estimate covariance matrix for `beetles` data
