...
Applying @def-coef-interp-procedure to the log-hazard linear predictor $\eta(\vx) = \diffloghaz(\vx) = \reglincomb$ (differencing with respect to $x_j$ with all other covariates held fixed) yields the following interpretation of $\b_j$:
