...
&= (\vx - \vxs) \cdot \beta && \text{(factoring out } \vb\text{)}
\ea
$$
...
&= (\vx - \vzero) \cdot \beta && \text{(applying @lem-diffloghaz-ph with } \vxs = \vzero\text{)} \\
...
&= \vx'\vb-\paren{\vxs}'\vb && \text{(substituting linear predictors)} \\
&= (\vx - \vxs)'\vb && \text{(factoring out } \vb\text{)}
\ea
$$
:::
