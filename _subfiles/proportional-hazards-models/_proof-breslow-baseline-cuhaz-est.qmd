...
for subject $j$.
Substituting
$\haz(t \mid \vx) = \haz_0(t)\,\theta(\vx)$ (by @thm-ph-haz-decomp)
and
$\surv(t \mid \vx) = \expf{-\cuhaz_0(t)\,\theta(\vx)}$
...
\Lik\sb{\b,\,\haz_0(\cdot)}
= \prod_{j = 1}^{n}
    \sb{\haz_0(\tilde{T}_j)\,\theta(\vx_j)}^{\delta_j}\,
    \expf{-\cuhaz_0(\tilde{T}_j)\,\theta(\vx_j)}.
$$
...
penalty $\expf{-\cuhaz_0(\tilde T_j)\theta(\vx_j)}$ is therefore
...
$$
\ba
\sum_j \cuhaz_0(\tilde{T}_j)\,\theta(\vx_j)
&= \sum_j \theta(\vx_j) \sum_{i:\ t_i \le \tilde{T}_j} h_{0i}
\\
&= \sum_i h_{0i} \sum_{j:\ \tilde{T}_j \ge t_i} \theta(\vx_j)
\\
&= \sum_i h_{0i} \sum_{j \in R(t_i)} \theta(\vx_j),
\ea
$$

where the second equality swaps the order of summation,
and the third uses $R(t_i) = \{j : \tilde{T}_j \ge t_i\}$
(see the [risk set definition](intro-to-survival-analysis.qmd#def-risk-set); this is the "at risk *at* $t_i$" convention with the
$\ge$ boundary — a subject censored exactly at $t_i$ is in $R(t_i)$
and contributes $h_{0i}$ to $\cuhaz_0(\tilde{T}_j)$).
Write $S_i = \sum_{j \in R(t_i)} \theta(\vx_j)$ for the
...
$$
\ba
\prod_{j=1}^n \sb{\haz_0(\tilde{T}_j)\,\theta(\vx_j)}^{\delta_j}
&= \prod_{j:\,\delta_j=1} \haz_0(\tilde{T}_j)\,\theta(\vx_j)
   && \text{(terms with } \delta_j = 0 \text{ equal } 1\text{, dropped)}
\\
&= \prod_{i=1}^D h_{0i}\,\theta(\vx_{(i)}).
...
&= \underbrace{\prod_{i=1}^{D} h_{0i}\,\theta(\vx_{(i)})}_{\text{hazard-density}}
...
\quad\text{where } S_i = \sum_{j \in R(t_i)} \theta(\vx_j).
$$
...
\\
&= e^{-D}
   \cdot \prod_{i = 1}^{D} \frac{\theta(\vx_{(i)})}{S_i}.
\ea
$$

Under the no-ties assumption each event time $t_i$ has $d_i = 1$, so
$\prod_{i=1}^D \frac{\theta(\vx_{(i)})}{S_i}$
