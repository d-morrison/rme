...
$$
\haz_0(t) = \begin{cases}
h_{0i}, & t = t_i \text{ for some } i \in \set{1, \dots, K},\\
0,      & \text{otherwise},
\end{cases}
$$
...
\\
&= \prod_{i=1}^K h_{0i}\,\hazfactor(\vx_{(i)}).
...
&= \underbrace{\prod_{i=1}^{K} h_{0i}\,\hazfactor(\vx_{(i)})}_{\text{hazard-density}}
   \cdot \underbrace{\prod_{i=1}^{K} \expf{-h_{0i}\,S_i}}_{\text{survival}}
\\
&= \prod_{i = 1}^{K}
...
= \sum_{i = 1}^{K} \log\hazfactor(\vx_{(i)})
  + \sum_{i = 1}^{K}
...
\\
&= e^{-K}
   \cdot \prod_{i = 1}^{K} \frac{\hazfactor(\vx_{(i)})}{S_i}.
\ea
$$

Under the no-ties assumption each event time $t_i$ has $d_i = 1$, so
$\prod_{i=1}^K \frac{\hazfactor(\vx_{(i)})}{S_i}$
