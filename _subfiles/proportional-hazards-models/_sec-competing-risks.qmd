...
# (no group argument, so there is one group; element names are "1 1" and "1 2")
mros_cif <- cuminc(
  ftime   = mros_data$years,
  fstatus = mros_data$status
)

# Extract CIF by cause code
# cuminc() names elements "group cause" (e.g., "1 1", "1 2");
# use endsWith() so the name is found by cause code regardless of group label.
# Since no group argument is given there is exactly one match per cause.
...
# Extract CIF estimates into data frame
fracture_cif <- tibble(
...
# Compute 5-year CIF values for inline reporting
fracture_cif_at_five_yr <-
  fracture_cif$cif[which.min(abs(fracture_cif$time - 5))]
death_cif_at_five_yr <-
  death_cif$cif[which.min(abs(death_cif$time - 5))]
...
- About `r round(fracture_cif_at_five_yr * 100, 1)`% of men had experienced a hip fracture
- About `r round(death_cif_at_five_yr * 100, 1)`% had died without a fracture
