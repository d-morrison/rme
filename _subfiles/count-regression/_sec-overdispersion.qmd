...
to equal the conditional mean ($\Var{Y \mid \vX=\vx} = \Expp[Y \mid \vX=\vx]$).
...
relative to a parametric model $\P(Y=y \mid \vX=\vx)$ if
its conditional empirical variance in a dataset exceeds
the theoretical variance imposed by the fitted model $\ep(Y=y \mid \vX=\vx)$.
