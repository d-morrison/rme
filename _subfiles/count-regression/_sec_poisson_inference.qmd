...
To compare a smaller model (with $p_0$ parameters) to a larger model
(with $p_1 > p_0$ parameters), use the likelihood ratio test statistic:

$$
G^2 = 2\bigl[\ell(\hat\beta_1) - \ell(\hat\beta_0)\bigr]
$$

Under $H_0$ that the additional $p_1 - p_0$ parameters are all zero,
$G^2 \dot\sim \chi^2_{p_1 - p_0}$.
