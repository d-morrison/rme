...
\eta(\vx) &= \tp{\vx}\vb = \beta_0 + \beta_1 x_1 + \dots + \beta_p x_p
\end{aligned}
$${#eq-mean-poisson}
...
&= \tp{\vx}\vb + \log{t} && \text{(substituting linear predictor } \eta(\vx) = \tp{\vx}\vb\text{)} \\
&= (\beta_0 + \beta_1 x_1 + \dots + \beta_p x_p) + \log{t} && \text{(expanding vector dot product)}
\ea
$$
