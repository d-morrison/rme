...
&= \logf{\lambda(\vx)} + \logf{t} && \text{(by logarithmic product rule } \logf{a \cdot b} = \logf{a} + \logf{b}\text{)} \\
&= \logf{\exp{\eta(\vx)}} + \logf{t} && \text{(substituting rate function } \lambda(\vx) = \exp{\eta(\vx)}\text{)} \\
&= \eta(\vx) + \logf{t} && \text{(by inverse relationship of log and exp)} \\
&= (\beta_0 + \beta_1 x_1 + \dots + \beta_p x_p) + \logf{t} && \text{(substituting linear predictor } \eta(\vx) = \beta_0 + \beta_1 x_1 + \dots + \beta_p x_p\text{)}
\ea
$$
