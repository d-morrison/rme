...
(i.e., $\underbrace{\deriv{\vb} \vx}_{p \times p} = \underbrace{\mathbf{0}}_{p \times p}$),
then:
...
:::: notes
Here $\deriv{\vb} \vx$ is the $p \times p$ Jacobian matrix in denominator layout:
entry $(i,j)$ equals $\partial x_j / \partial \beta_i$.
No transpose on $\vb$ or $\vx$ is needed.
::::
$$
...
If $A$ is a constant $m \times p$ matrix
(constant with respect to $\vb$),
then:
