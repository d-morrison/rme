...
In non-parametric survival analysis, $\cuhaz(t)$ is tractable to estimate, and we can then
...
with rate parameter $\haz$ is:

$$
\cuhaz(t) = \mathbb{1}_{t \ge 0}\cdot \haz t
$$
...
ggplot() +
  ...
  geom_abline(
    aes(col = "1", intercept = 0, slope = 1)
  ) +
  geom_abline(
    aes(col = "1.5", intercept = 0, slope = 1.5)
  ) +
  geom_abline(
    aes(col = "5", intercept = 0, slope = 5)
  ) +
  theme_bw() +
  ylab(expr(Lambda)) +
  ylim(0, 5) +
