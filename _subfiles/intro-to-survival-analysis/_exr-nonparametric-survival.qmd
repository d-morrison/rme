...
- $\hat{\cs}(y) = \hat{\Pr}(Y > y \mid Y \geq y) = [r(y) - d(y)]/r(y)$: conditional survival factor;
- $\hat{\haz}(y) = \hat{\Pr}(Y = y \mid Y \geq y) = \frac{d(y)}{r(y)} = 1 - \hat{\cs}(y)$: estimated discrete hazard;
- $\hsurv_{KM}(y) = \prod_{k:\, y_k \leq y} \hat{\cs}(y_k)$: Kaplan-Meier survival estimate.
