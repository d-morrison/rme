...
&= \prod_{t_i \le t}\hat{\kappa}^{NA}_i
\ea
$$

where $\hat{\kappa}^{NA}_i = \expf{-\hat{\lambda}_i} = \expf{-d_i/r_i}$
is the Nelson-Aalen conditional survival factor
(compare with the KM factor $\hat{\kappa}^{KM}_i = 1 - d_i/r_i$).
...
\hsurv_{KM}(t) &= \prod_{t_i \le t}\sb{1-\frac{d_i}{r_i}}
\ea
$$
