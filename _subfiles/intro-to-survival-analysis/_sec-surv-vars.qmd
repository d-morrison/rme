...
-   Often, censored observations are **right-censored**,
...
**Right censoring** occurs when the observed follow-up time is less than the true event time — we know only that the event has not yet occurred by the time of last contact.
...
**Left censoring** occurs when the event is known to have already happened before the first observation time — we know only that the event time is less than some observed time $t$.

-   An individual tests positive for HIV.
-   If the event is infection with HIV, then we only know that it has
occurred before the testing time $t$, so this observation is left censored.
...
**Interval censoring** occurs when the event is known to have occurred within a specific time interval $(t_1, t_2]$, but the exact event time is unobserved.
