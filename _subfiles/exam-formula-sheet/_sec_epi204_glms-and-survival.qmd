...
| $\Delta\eta(\vx) = \vx\cdot\vb = \beta_1 x_1 + \cdots + \beta_p x_p$ | Linear predictor |
| $\theta(\vx) = \expf{\Delta\eta(\vx)}$ | Hazard multiplier |
| $\defHR$ | Hazard ratio definition |
| $\hr(t\mid\vx:\vxs) = \expf{\Delta\eta(\vx) - \Delta\eta(\vxs)} = \expf{(\vx-\vxs)\cdot\vb}$ | Hazard ratio formula |
