...
In clinical trial research, the concept of an estimand has taken on particular importance following the ICH E9(R1) addendum [@nejm2024estimand]. A clear specification of the estimand ensures that the clinical question of interest aligns with the statistical analysis plan.

For clinical trials, an estimand consists of several key attributes:

1. **Population**: The patients targeted by the clinical question
2. **Variable (endpoint)**: The outcome measurement of interest
3. **Population-level summary**: How the variable is summarized across patients (e.g., mean difference, odds ratio)
4. **Intercurrent events**: Events that occur after treatment initiation that may affect the interpretation of the results (e.g., treatment discontinuation, use of rescue medication, death)
5. **Strategy for intercurrent events**: How these events are handled in the analysis

Common strategies for handling intercurrent events include:

- **Treatment policy strategy**: Include all participants regardless of intercurrent events
- **Composite strategy**: Combine the intercurrent event with the primary outcome
- **Hypothetical strategy**: Estimate what would have happened if the intercurrent event had not occurred
- **Principal stratum strategy**: Focus on the subset of patients who would not experience the intercurrent event
- **While on treatment strategy**: Only consider measurements while on assigned treatment

By precisely defining the estimand before collecting data, researchers ensure that the trial design, conduct, and analysis address a meaningful clinical question [@pohl2021estimands; @lawrance2020estimand].
