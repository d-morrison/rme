...
\haz(t|\boldsymbol x) &= \exp{\eta(\boldsymbol x)}\\
h_0 &\stackrel{\text{def}}{=} \haz(t|\boldsymbol 0)\\
...
$$\surv(t|X=x)=S_0(t\cdot \theta(x))$$ where $\theta(x)=\exp{\eta(x)}$,
...
$$
\begin{aligned}
\cuhaz(t|x) &= H_0(\theta(x)\cdot t)\\
\haz(t|x) &= \theta(x) \cdot h_0(\theta(x)\cdot t)
\end{aligned}
$$
