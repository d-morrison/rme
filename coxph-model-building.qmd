...
::: {.callout-important}
## Caution: Variable Selection and Collider Bias

When building Cox proportional hazards models (or any regression model),
carefully consider which covariates to include based on subject matter knowledge and causal reasoning.

Including a variable that is influenced by both the exposure and outcome (a "collider") can introduce bias rather than control for confounding.
For example,
in a study of treatment effects on survival,
including intermediate outcomes or complications that occur after treatment as covariates could bias the treatment effect estimate.

See @sec-collider-bias for more details on collider bias and how to avoid it.
The [Catalog of Bias](https://catalogofbias.org/) provides additional resources on this and other types of bias.
:::
