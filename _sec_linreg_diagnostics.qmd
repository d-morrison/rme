...
Since $\err(Y)$ is an affine function of $Y$,
...
(each observation has diminishing influence on its own fitted value),
...
where $h_{ii}$ is the $i$-th diagonal element of the hat matrix.
For large $n$, $h_{ii} \approx 0$, giving the approximation:

$$e_i \approx \Normal(0, \ss) \approx \Normal(0, \hat\ss)$$ {#eq-resid-dist-approx}
