...
Derive interpretations of $\beta_0$, $\beta_P$, $\beta_A$, and $\beta_{PA}$ on the odds **and** log-odds scales, 
...
$$
\beta_{0} = \logitf{P(Y=1|P=0,A=0)}
$$

$\beta_{0}$ is the log of the odds ("log-odds") of experiencing CHD for a `r age_offset` year-old person with a type B personality; that is,
...
$$
\ba
\deriv{a}\eta(P=0, A = a) &= \deriv{a} (\beta_0 + \beta_P 0 + \beta_A a + \beta_{PA}(0\cdot a))
\\
...
\begin{aligned}
\beta_{P}
&= \logit\{\Pr(Y= 1|P=1, A = 0)\}\\
&\ \ \ \  - \logit\{\Pr(Y= 1| P=0, A = 0)\}
\end{aligned}
$$
