...
For each fixed value $y$ with positive probability or density:

**Discrete case.**
Conditioning on $Y=y$,
and applying the law of total probability
to the partition $\{Z=z : z \in \rangef{Z}\}$
under the conditional distribution given $Y=y$:

$$
\ba
\E{\E{X \mid Y,Z} \mid Y=y}
&= \sum_{z \in \rangef{Z}} \E{X \mid Y=y,Z=z} \cd \P(Z=z \mid Y=y)
\\&= \E{X \mid Y=y}
\ea
$$
...
under the conditional density $\p(Z=z \mid Y=y)$:

$$
\ba
\E{\E{X \mid Y,Z} \mid Y=y}
&= \int_{z \in \rangef{Z}} \E{X \mid Y=y,Z=z} \cd \p(Z=z \mid Y=y)\, dz
\\&= \E{X \mid Y=y}
\ea
$$

Therefore,
as random variables of $Y$,
$\E{X \mid Y} = \E{\E{X \mid Y,Z} \mid Y}$.
