...
:::{#def-error}
### Error/noise in probabilistic models

In probabilistic models,
an **error** (or **noise term**)
is a deviation of an observation from its model-implied population mean.
...
$\err_i \eqdef y_i - \E{Y_i \mid X_i}$.

The terms "error" and "noise" are common,
but terminology is not fully consistent across sources.
In this course,
we use "residual" for deviations from fitted values
(defined in the
[Estimation chapter](estimation.qmd#def-residual)),
not for deviations from population means.
