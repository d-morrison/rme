...
If $B_1, B_2, \ldots, B_n$ is a partition of the sample space
(i.e., mutually exclusive events whose union is the entire sample space),
then for any event $A$:

$$\Pr(A) = \sum_{i=1}^{n} \Pr(A \mid B_i) \cd \Pr(B_i)$$
...
Since $B_1, \ldots, B_n$ partition the sample space,
the events $A \cap B_1, \ldots, A \cap B_n$ are mutually exclusive and their
union is $A$.
By repeated application of property 3 of @def-probability,
...
\\&= \sum_{i=1}^{n} \Pr(A \mid B_i) \cd \Pr(B_i)
\ea
$$
...
denoted $\E{Y \mid X = x}$,
is the expected value of $Y$ restricted to the subpopulation where $X = x$.

For discrete $Y$:

$$\E{Y \mid X = x} = \sum_{y \in \rangef{Y}} y \cd \P(Y = y \mid X = x)$$

For continuous $Y$:

$$\E{Y \mid X = x} = \int_{y \in \rangef{Y}} y \cd \p(Y = y \mid X = x)\, dy$$

The **conditional expectation function** $\E{Y \mid X}$ is the random variable
obtained by evaluating $\E{Y \mid X = x}$ at the realized value of $X$.
...
For any two *discrete* random variables $X$ and $Y$ with finite ranges:
...
treating the finite partition $\{X = x : x \in \rangef{X}\}$
by repeated application of property 3 of @def-probability:
