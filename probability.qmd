...
Given order statistics $x_{(1)}\le\cdots\le x_{(n)}$,
the sample quantile function is
$$\hat Q(p)=x_{(i)} \quad \text{for } p\in((i-1)/n,i/n],\ \ i=1,\ldots,n.$$
This is the generalized inverse of the [empirical CDF](estimation.qmd#def-empirical-cdf).
