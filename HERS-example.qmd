...
Recall from previous sections that the asymptotic standard error of ${\widehat{\mu}}_{ML}$ is $\widehat{\text{SE}}\left({\widehat{\mu}}\right) = \sqrt{\left[\left(\hat{\mathcal{I}}\left(\widehat{\mu}_{ML}\right)\right)^{-1}\right]} = \frac{\hat{\sigma}}{\sqrt{n}}$
...
For example, suppose we wish to detect a difference from the hypothesized value $\mu_0 = 95$.We reject the null hypothesis for any value outside the interval 
$$ \mu_0 \pm \Phi^{-1}(0.975) \sqrt{\frac{\sigma^2}{n}} $$ NOTE: this is for the simplest case when $\sigma$ is known.

```{r}
mu_0 = 95
se = se.mu.hat(n = 100)
margin = qnorm(0.975) * se
#qt(0.975,nrow(hers)-1)
upperbound = mu_0 + margin
lowerbound = mu_0 - margin
```

In this case, the interval is $[`r lowerbound`, `r upperbound`]$.

#### Calculate power under a simple alternative

Against the simple alternative that the true value is actually the estimated mean calculated from the data (i.e.$`r mu_hat`$). Let's also assume that the known standard deviation is what we estimated from the data.
...
power = function(n = 100, null = 95, alt = 98.66){
  ...
  se_null = se.mu.hat(n = n)
  reject_upper = (null + qnorm(0.975) * se_null)
  reject_lower = (null - qnorm(0.975) * se_null)
  p_reject_high =
    pnorm(
      q = reject_lower,
      mean = alt,
